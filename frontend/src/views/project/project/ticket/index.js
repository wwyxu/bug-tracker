...
import { deleteProjectTicket } from "../../../../services/api/project";
import { addTicket } from "../../../../services/api/ticket";
  ...
  render() {
    ...
    const tickets = ticket.map((ticket) => (
      <tr key={ticket._id._id}>
        ...
        <td>{moment(ticket._id.date).format("DD/MM/YYYY")}</td>
        <td>
          {ticket._id.duedate
            ? moment(ticket._id.duedate).format("DD/MM/YYYY")
            : null}
        </td>
        <td>
